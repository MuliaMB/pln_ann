...
<div class="container">
    ...
    <form action="{{ route('table_data_penyulangs.update', $table_data_penyulang->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select name="id_penyulang" id="id_penyulang" class="form-control" required>
                ...
                @foreach($penyulangs as $penyulang)
                    <option value="{{ $penyulang->id }}" {{ (old('id_penyulang', $table_data_penyulang->id_penyulang) == $penyulang->id) ? 'selected' : '' }}>
                        {{ $penyulang->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="{{ $field['type'] }}" 
                   ...
                   id="{{ $field['name'] }}" 
                   name="{{ $field['name'] }}" 
                   value="{{ old($field['name'], $table_data_penyulang->{$field['name']}) }}" 
                   required>
        </div>
        @endforeach
